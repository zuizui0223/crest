...
def test_four_region_factorization_sign_with_positive_balance() -> None:
    # left-only={0}, right-only={1}, overlap={2}, residual={3}; q-power
    # balance is positive for this occupancy choice over the tested orders.
    ...
    for q in (0.0, 0.2, 0.7):
        ...
        if q == 0.0:
            # Equal region cardinalities put q=0 exactly on the balance surface.
...
def test_nonshannon_zero_leakage_exists_on_exact_balance_surface() -> None:
    # One equally occupied cell in each of the four regions gives W_q Z_q = X_q Y_q
    # for every finite q.  Thus overlap destroys global Shannon uniqueness.
